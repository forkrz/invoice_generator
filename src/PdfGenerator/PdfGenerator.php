...
class PdfGenerator
{
    ...
    public function __construct(MyPdf $pdf)
    {
        $this->pdf = $pdf;
    }
    ...
    {
        $this->pdf->setDateOfIssue(Carbon::today()->format('d/m/Y'));
        $this->pdf->setRealisedOn(Carbon::today()->format('d/m/Y'));


        $this->pdf->test($userData, $formData);
//        $title =
//
...
//        //product table
//
//        $pdf->writeHTMLCell(0, 0, '', '', $pdf->createTable($formData), 0, 1, 0, true, '', true);
//
//
